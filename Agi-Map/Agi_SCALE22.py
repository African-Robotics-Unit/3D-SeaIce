...
m.drawparallels(parallels,labels=[False,True,False,False],fontsize=10,color='#808080') # labels = [left,right,top,bottom]
meridians = np.arange(0.,360.,0.2)
m.drawmeridians(meridians,labels=[False,False,False,True],fontsize=10,color='#808080')
...
cb.set_label('AMSR2 Sea Ice Concentration [%] on 20.07.2022', fontsize=15)
...
dfStations = pd.read_excel(file1, sheet_name='POI')
...
m.plot(xStations, yStations, 'y.', markersize=5,label='Stations') 
...
fnt=10
ax.annotate('P1&P2', xy=(xStations[0], yStations[0]), xycoords='data', xytext=(xStations[0]-6000, yStations[0]-6000), 
            textcoords='data',fontsize=fnt,color='y')
ax.annotate('P3', xy=(xStations[1], yStations[1]), xycoords='data', xytext=(xStations[1]+6000, yStations[1]+6000), 
            textcoords='data',fontsize=fnt,color='y')
ax.annotate('P4', xy=(xStations[2], yStations[2]), xycoords='data', xytext=(xStations[2]-6000, yStations[2]-6000), 
            textcoords='data',fontsize=fnt,color='y')
ax.annotate('SB06', xy=(xStations[3], yStations[3]), xycoords='data', xytext=(xStations[3]-6000, yStations[3]-6000), 
            textcoords='data',fontsize=fnt,color='y')
...
dfA = pd.read_excel(file2, sheet_name='SB6 ICE00')
dfA = dfA.dropna()
lonA = dfA['Lon'].tolist()
latA = dfA['Lat'].tolist()
xA,yA = m(lonA, latA)
m.plot(xA, yA, 'm-', linewidth=3, label='SB6 ICE00') #'mo-' indicates m = magenta(change for different colours), o=dot, -=line joining points
m.plot(xA[0], yA[0], 'm*', markersize=10,markeredgecolor='k')
...
leg =ax.legend(loc='upper center',bbox_to_anchor=(0.5,-0.2),frameon=True, ncol=5,prop={'size':13})
