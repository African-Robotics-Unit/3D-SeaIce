function [output] = interestPeaks_new(F,X,Y,n,cutoff)
%INTERESTPEAKS Using surface interpolation, calculates key pekas and
%troughs using the Rayleigh criterion 
X_interest_all = [];
Y_interest_all = [];
Z_interest_all = [];
...
for i = 1:n
    ...
    % Save all local maxima for this row (not just the top one)
    for m = 1:length(idxMaxSorted)
        allLocalMax_X = [allLocalMax_X, X(i, originalIndices(idxMaxSorted(m)))];
        allLocalMax_Y = [allLocalMax_Y, Y(i, originalIndices(idxMaxSorted(m)))];
        allLocalMax_Z = [allLocalMax_Z, zMaxSorted(m)];
    end
